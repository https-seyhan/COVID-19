...
def get_posteriors(ma, newtotalratio, sigma=0.15):
    ...
    print("R_T Shape ", r_t_range[:, None].shape)
    print("Ratio Shape ", newtotalratio[:, None].shape)

    #lam = ma[:-1].values * np.exp(GAMMA * (r_t_range[:, None] - 1) + newtotalratio[:, None].T)
    lam = ma[:-1].values * np.exp(GAMMA * (r_t_range[:, None] - 1))
    #lam = ma[:-1].values * np.exp(GAMMA * (r_t_range - 1) + newtotalratio[:, None])
    #powercalc = np.exp(GAMMA * ((r_t_range[:, None] - 1)  + newtotalratio[:, None]))
    powercalc = np.exp(GAMMA * (r_t_range[:, None] - 1))
    sumtwovecs = np.exp(GAMMA * ((r_t_range[:, None] - 1) + newtotalratio[:, None]))
    lam = ma[:-1].values * sumtwovecs
    print("Sum Vecs ", sumtwovecs )
    print("Sum Vecs Shape", sumtwovecs.shape)
    print("power Calc Shape", powercalc.shape)
    print("Lambda Shape", lam.shape)
    #print("Stats on powercalc ", stats.describe(powercalc))
    print("Transposed New Total Ratio ", newtotalratio[:, None].T)
    print(" Non - Transposed New Total Ratio  ", newtotalratio[:, None])
    print("R_T  ", r_t_range[:, None])
    print(" Transposed New Total Ratio Type ", type(newtotalratio[:, None].T))
    print(" Non - Transposed New Total Ratio Type ", type(newtotalratio[:, None]))
