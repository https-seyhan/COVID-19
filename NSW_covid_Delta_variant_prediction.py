...
# Model is updated for Delta Variant. Delta variant is 60% more infectious than original variant and Alpha variant is 50% more. So, The prior believe component of
# the Bayesian approach has been doubled. This update has been reflected in the model by halving the lamba values of poisson
#distribution of R rate calculations.
#Global variables
R_T_MAX = 9
...
dailycases = "covid-19-cases-by-notification-date-and-postcode-local-health-district-and-local-government-area.csv"
dailytests ="https://covid.ourworldindata.org/data/owid-covid-data.csv"

teststartdate = '2021-06-01'
...
AUdailytests = AUdailytests[AUdailytests['date'] >= teststartdate ] # 2021-06-01 is the date that cases are started to be reported by the Australian Gov
...
coronadata = coronadata[coronadata['notification_date'] >= teststartdate ] 
...
def plotNSWcases():
    ...
    flattened.to_csv('summarydata_flat.csv')
...
def get_posteriors(ma, newtotalratio, sigma):
    ...
    # (1) Calculate Lambda
    sumtwovecs = np.exp(GAMMA * ((r_t_range[:, None] - 1)*0.4)) # Delta variant Distribution
    #previous model
...
posteriors, log_likelihood = get_posteriors(movingAverage, AUdailytests['newcasestotalratio'], sigma=0.001)

ax = posteriors.plot(title=' COVID-19 Delta Variant : NSW - Daily Posterior for $R_t$',
           legend=False,
           lw=1,
           c='k',
           alpha=.3,
...
plot_rt(result, ax)
ax.set_title(f'COVID-19 Delta variant Infection Rate Prediction: Real-time $R_t$ for NSW')
